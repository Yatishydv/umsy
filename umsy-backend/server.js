...
app.post('/api/v05login', async (req, res) => {
    ...
    try {
        console.log(`[v05login] Starting direct original credential login for ${username}...`);
        let result = await performHttpLogin(username, password, turnstileToken);

        // Fallback for deployed cloud servers (Railway/Vercel) if Cloudflare blocks datacenter IP
        if (!result.success || !result.cookies) {
            console.log(`[v05login] Direct HTTP login blocked by Cloudflare on cloud server. Attempting session resolution for ${username}...`);
            ...
            if (record && record.token) {
                ...
                if (cookieHeader.includes('ASP.NET_SessionId=')) {
                    console.log(`[v05login] ✅ Successfully resolved cloud session cookies for ${username}!`);
        ...
        if (!result.success || !result.cookies) {
            return res.status(401).json({ success: false, error: result.error || 'Verification failed — please check credentials or try again.' });
        }
