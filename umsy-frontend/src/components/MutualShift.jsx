    ...
    return (
        <div className="flex flex-col gap-6">
                    ...
                    <div className="flex items-center justify-between pt-2">
                        <div>
                            <h1 className="text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">Hostel Board</h1>
                            <p className="text-xs text-slate-400 dark:text-zinc-500 mt-0.5">Your hostel info &amp; mutual shift board</p>
                        </div>
                    </div>

                    {/* ── Hostel Info card ── */}
                    {hostelInfo && (
                        <div className="bg-white dark:bg-zinc-800 border border-slate-200/60 dark:border-zinc-800 rounded-2xl px-5 py-4 flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-zinc-700 flex items-center justify-center flex-shrink-0">
                                <Home className="h-5 w-5 text-slate-500 dark:text-zinc-400" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-bold text-slate-900 dark:text-slate-100 truncate">{hostelInfo.name}</p>
                                <p className="text-xs text-slate-500 dark:text-zinc-400 truncate">
                                    {hostelInfo.hostel} &bull; Room&nbsp;{hostelInfo.roomNo}
                                </p>
                            </div>
                            <div className="text-right flex-shrink-0">
                                <p className="text-[10px] text-slate-400 dark:text-zinc-500 uppercase tracking-widest font-bold">VID</p>
                                <p className="text-sm font-mono font-semibold text-slate-700 dark:text-slate-200">{hostelInfo.vid}</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex bg-slate-100 dark:bg-zinc-800 border border-slate-200/30 rounded-full p-1.5 gap-1 max-w-sm">
                        {[
                            ...
                            <button
                                ...
                                className={`cursor-pointer flex-1 flex items-center justify-center gap-1.5 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all ${activeTab === id ? 'bg-[#bef227] text-[#1c312e] shadow-sm' : 'text-slate-500 dark:text-zinc-400'}`}
                            >
                                <Icon className="h-4 w-4" />
                                {label}
                                {id === 'browse' && allPosts.length > 0 && (
                                    <span className="ml-1.5 text-[10px] bg-[#1c312e] text-white rounded-full px-1.5 py-0.5 font-bold">
                                        {allPosts.length}
                                    </span>
                                )}
                            </button>
