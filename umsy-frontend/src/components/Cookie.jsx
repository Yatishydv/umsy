...
const Cookie = () => {
    ...
    const handleContinue = async () => {
        ...
        localStorage.setItem('umsy_cookies', cookie);
        localStorage.removeItem('umsy_is_v04');
        
        // Save to backend database for server tasks
        const regno = localStorage.getItem('umsy_regno');
        if (regno) {
            try {
                await saveSession(regno, cookie);
            } catch (err) {
                console.warn('⚠️ Could not save session to database:', err.message);
            }
        }
    ...
    const copyBookmarklet = () => {
        const code = "javascript:(function(){const val=document.cookie;if(val){navigator.clipboard.writeText(val).then(()=>{alert('All session cookies copied to clipboard!');}).catch(()=>{prompt('Copy cookies manually:',val);});}else{alert('No cookies found!');}})();";
                ...
                <div className="bg-card/40 backdrop-blur-xl border border-border/50 rounded-[2.5rem] p-8 md:p-10 shadow-2xl shadow-black/10 space-y-6">
                    ...
                    <div className="text-center space-y-2">
                        ...
                        <p className="text-muted-foreground text-sm leading-relaxed">
                            Import your session cookies to securely access the UMSY dashboard.
                        </p>
                    </div>
