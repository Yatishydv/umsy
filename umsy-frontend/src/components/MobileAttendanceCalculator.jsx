...
const MobileAttendanceCalculator = () => {
    ...
    const [globalDates, setGlobalDates] = useState(() => {
        const cached = localStorage.getItem('umsy_mobile_calc_global_dates');
    ...
    useEffect(() => {
        localStorage.setItem('umsy_mobile_calc_global_dates', JSON.stringify(globalDates));
    }, [globalDates]);

    useEffect(() => {
        if (Object.keys(courseInputs).length > 0) {
            localStorage.setItem('umsy_mobile_calc_inputs', JSON.stringify(courseInputs));
        }
    }, [courseInputs]);
    ...
    useEffect(() => {
        ...
        const studentInfoRaw = localStorage.getItem('umsy_student_info');
        if (studentInfoRaw) {
            try {
        ...
        if (cachedData) {
            ...
            const cachedInputs = localStorage.getItem('umsy_mobile_calc_inputs');
            let initialInputs = {};
            if (cachedInputs) {
    ...
    const realAttendence = localStorage.getItem('umsy_student_info')
